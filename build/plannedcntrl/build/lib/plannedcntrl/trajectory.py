...
def plan_curved_trajectory(target_position):
    ...
    # Create unique temporary node
    node_name = f"trajectory_planner_{int(time.time()*1000)}"
    node = rclpy.create_node(node_name)

    tf_buffer = tf2_ros.Buffer()
    tf_listener = tf2_ros.TransformListener(tf_buffer, node)

    node.get_logger().info("Waiting for TF transform (odom -> base_link)...")

    # Allow TF listener to fill buffer
    ...
    while rclpy.ok():
        try:
            ...
            if time.time() - start_time > 5.0:
                node.get_logger().warn("Still waiting for TF data...")
            continue

    if trans is None:
        node.get_logger().error("Failed to get odom->base_link transform.")
        node.destroy_node()
        return []
    ...
    while rclpy.ok():
        try:
            trans = tf_buffer.lookup_transform('odom', 'base_link', rclpy.time.Time()) ## Apply a lookup transform between our world frame and turtlebot frame
            break
        except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException):
    ...
    x2 = x1 + (dx * math.cos(yaw) - dy * math.sin(yaw)) ## How would you get x2 from our target position? Remember this is relative to x1
    y2 = y1 + (dx * math.sin(yaw) + dy * math.cos(yaw)) ## How would you get x2 from our target position? Remember this is relative to x1
    ...
    node.get_logger().info(f"Planned {len(waypoints)} waypoints from ({x1:.2f}, {y1:.2f}) → ({x2:.2f}, {y2:.2f})")
    node.destroy_node()
    return waypoints
