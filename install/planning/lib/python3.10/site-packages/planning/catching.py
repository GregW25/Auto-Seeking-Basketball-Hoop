...
# Import your IK Planner
from planning.ik import IKPlanner

# --- CONFIGURATION ---
TABLE_SAFETY_HEIGHT = 0 # Robot will ignore any ball below this Z height (meters)

class UR7e_HoopCatch(Node):
    def __init__(self):
        super().__init__('hoop_catch_node')

        # Subscribe to the MARKER topic to get the red sphere position
        ...
        # Non-blocking wait for server at startup
    ...
    def impact_callback(self, msg: Marker):
        # 1. CRITICAL FIX: Ignore 'DELETE' markers (which act as cleanup signals)
        ...
        # --- OFFSET APPLIED HERE ---
        target_point.y -= 0.2  # Shift 1cm in the negative Y direction
        # ---------------------------
        
        # 4. TABLE SAFETY CHECK (New)
        ...
        # 5. Compute IK (Get Target Joint Angles)
