...
class IKPlanner(Node):
    ...
    def compute_ik(self, current_joint_state, x, y, z,
        ...
        pose.pose.position.x = x  # Set the target x position
        pose.pose.position.y = y  # Set the target y position
        pose.pose.position.z = z  # Set the target z position
        ...
        # Lookup the format for ik request and build ik_req by filling in necessary parameters. What is your end-effector link name?
        ...
        ik_req.ik_request.pose_stamped = pose  # Set the target pose

        # Add the current joint state to the request for a starting configuration
